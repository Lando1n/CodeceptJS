    ...
    recorder.add(
      ...
      async () => {
        ...
        try {
          ...
          // Track all screenshots for the test including sessions and
          if (!test.artifacts.screenshots) test.artifacts.screenshots = []
          test.artifacts.screenshots.push(test.artifacts.screenshot)
          ...
          if (helper.activeSessionName) {
            ...
            for (const sessionName in sessions) {
              ...
              const fullScreenShotPath = path.join(global.output_dir, screenshotFileName)
              test.artifacts.screenshots.push(fullScreenShotPath)
              test.artifacts[`${sessionName.replace(/ /g, '_')}_screenshot`] = fullScreenShotPath
              allureReporter?.addAttachment(`${sessionName} - Last Seen Screenshot`, fs.readFileSync(fullScreenShotPath), dataType)
            }
          }
